...
def previous_info(ticker):
    ...
    try:
        ...
        for idx, row in enumerate(annual_eps_table.find_all('tr')):
            if idx > 3:
                break
        ...
        # 四半期EPSデータを取得
        for idx, row in enumerate(quarter_eps_table.find_all('tr')):
            if idx > 6:
                break
            cols = row.find_all('td')
            if len(cols) >= 2:
    ...
    # Revenueの値を取得
    try:
        driver.get(revenue_url)
        print(f"driver got {time.time() - start}")
        time.sleep(1.0)
        # ページのHTMLをBeautiful Soupで解析
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for idx, row in enumerate(annual_revenue_table.find_all('tr')):
            if idx > 3:
                break
            cols = row.find_all('td')
            if len(cols) >= 2:
        ...
        # 四半期Revenueデータを取得
        for idx, row in enumerate(quarter_revenue_table.find_all('tr')):
            if idx > 6:
                break
            cols = row.find_all('td')
            if len(cols) >= 2:
...
# 個別株の終値データ
# stock_dir  = glob.glob(os.getcwd()+"/Stock_Trade/StockData*/*.csv", recursive=True)[:10]
...
for i in range(0, 5):
    ticker = stock_df["Ticker"][i]
    analyst_info(ticker)
    print(f"analyst info {time.time() - start}")
    previous_info(ticker)
    print(f"analyst info {time.time() - start}")
...
col = ["Ticker", "Previous Quarter Eps2", "Previous Quarter EPS", "Corrent Quarter EPS", "Next Quarter EPS",
            ...
            "Previous Annual Revenue2", "Previous Annual Revenue", "Corrent Annual Revenue", "Next Quarter Revenue"]
